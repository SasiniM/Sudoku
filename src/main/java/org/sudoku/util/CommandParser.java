...
public class CommandParser {
    ...
    public static Command parse(String command) {
        ...
        if (splitCommand.length == 1) {
            return commandBuilder.action(Action.of(splitCommand[0].toLowerCase())).build();
        } else if (splitCommand.length == 2) {
            String cellRef = splitCommand[0].toUpperCase();
            if (!SudokuValidation.isValidCellReference(cellRef)) {
                throw new SudokuGenericException("Invalid cell reference. Cell reference should be within A1- I9.");
            }
            commandBuilder.cellRef(cellRef);

            if (Action.CLEAR_NUM.getCode().equalsIgnoreCase(splitCommand[1])) {
                commandBuilder.action(Action.of(splitCommand[1].toLowerCase())).build();
            } else {
                commandBuilder.action(Action.MOVE_NUM);
                if (!splitCommand[1].matches("[1-9]")) {
                    throw new SudokuGenericException("Invalid number. Enter a value from 1 to 9.");
                }
            ...
            throw new SudokuGenericException("Invalid command format. Examples: B3 7, C5 clear, hint, check, quit");
        }
